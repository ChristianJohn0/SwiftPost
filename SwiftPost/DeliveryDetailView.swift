...
struct DeliveryDetailView: View {
    ...
    var body: some View {
        ...
        .colorScheme(.dark)
        .onAppear() {
            TrackerAPI().getTrackingDataFromAPI(from: url) { (response) in
                self.model = response[0]
...
//        ProgressSwiftUIView(code: "MW517419064CA", service: DeliveryService.CanadaPost)
//        ProgressSwiftUIView(code: "0104499000670842", service: DeliveryService.CanadaPost)
        DeliveryDetailView(code: "955110620106", service: DeliveryService.FedEx)
    }
}
